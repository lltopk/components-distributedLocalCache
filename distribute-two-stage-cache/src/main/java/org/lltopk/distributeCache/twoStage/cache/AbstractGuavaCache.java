...
import org.lltopk.distributeCache.common.utils.SpringUtil;
...
//    }
    ...
    @Override
    public void removeLocalKey(K key) {
...
//        }
        ...
        if (StringUtils.equals(key,"refresh")) {
            ...
            // 避免消息回声。
           cache.invalidateAll();
           removeRedisKeys(Lists.newArrayList("","",""));
        } else if (Objects.nonNull(key)) {
            // 否则尝试移除特定key
            removeLocalKey((K) key);
            removeRedisKeys(Lists.newArrayList(key));
        }
        log.info("entries after listener{}", entries());
    }

    private Boolean removeRedisKeys(List<String> keys){
    ...
    private StringRedisTemplate getRedisTemplate() {
        ...
        stringRedisTemplate = SpringUtil.getBeanByApplicationContextAware(StringRedisTemplate.class);
        return stringRedisTemplate;
    }
